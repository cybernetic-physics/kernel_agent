...
#!POPCORN gpu NVIDIA
#!AFTERHOURS harness tools/pygpubench_nvfp4_silu_dual_gemm_harness.py
...
def silu_intrinsic(src_A, singularity=False, loc=None, ip=None):
    ...
    inputs.append(
        llvm.extractelement(
            src_A, arith.constant(Int32.mlir_type, 0, loc=loc, ip=ip), loc=loc, ip=ip
        )
    )
    ...
    cons = "=f,f"
    res = llvm.inline_asm(
        llvm.StructType.get_literal([Float32.mlir_type] * 1),
        inputs,
        asm,
        cons,
        loc=loc,
    ...
    return vector.from_elements(
        ir.VectorType.get([1], Float32.mlir_type, loc=loc), out, loc=loc, ip=ip
    )
...
class DualGemm:
    ...
    def __call__(
        ...
        @cute.struct
        class SharedStorage:
            ab1_full_mbar_ptr: cute.struct.MemRange[
                cutlass.Int64, self.num_ab_stage * 2
            ]  # need both full & empty bar
            b2_full_mbar_ptr: cute.struct.MemRange[
                cutlass.Int64, self.num_ab_stage * 2
            ]  # need both full & empty bar
            acc1_full_mbar_ptr: cute.struct.MemRange[
                cutlass.Int64, self.num_acc_stage
            ]  # only need full bar
            acc2_full_mbar_ptr: cute.struct.MemRange[
                cutlass.Int64, self.num_acc_stage
            ]  # only need full bar
    ...
    def kernel(
        ...
        if warp_idx == self.mma_warp_id:
            ...
            for k_tile in cutlass.range(k_tile_cnt - self.mma_ep_disp_len):
                if is_leader_cta:
                    ...
                    b2_pipeline.consumer_wait(ab_consumer_state)
                    cute.copy(
                        tiled_copy_s2t_sfb2,
            ...
            for k_tile in cutlass.range(
                k_tile_cnt - self.mma_ep_disp_len, k_tile_cnt, unroll_full=True
            ):
                if is_leader_cta:
        ...
        if warp_idx < self.mma_warp_id:
            ...
            for subtile_idx in cutlass.range_constexpr(
                self.ep_pc_tile, unroll_full=True
            ):
            ...
            for subtile_idx in cutlass.range(
                self.ep_pc_tile, subtile_cnt, unroll_full=True
            ):
                tTR_tAcc1_mn = tTR_tAcc1[(None, None, None, subtile_idx)]
                tTR_tAcc2_mn = tTR_tAcc2[(None, None, None, subtile_idx)]
                tTR_rAcc1 = tTR_rAcc1_base[(None, None, None, subtile_idx)]
...
def compile_kernel(problem_size):
    ...
    b1_ptr = make_ptr(cutlass.Float4E2M1FN, 0, cute.AddressSpace.gmem, assumed_align=16)
    b2_ptr = make_ptr(cutlass.Float4E2M1FN, 0, cute.AddressSpace.gmem, assumed_align=16)
    ...
    _compiled_kernel_cache[problem_size] = cute.compile(
        gemm, a_ptr, b1_ptr, b2_ptr, sfa_ptr, sfb1_ptr, sfb2_ptr, c_ptr, problem_size
    )
    return _compiled_kernel_cache[problem_size]
